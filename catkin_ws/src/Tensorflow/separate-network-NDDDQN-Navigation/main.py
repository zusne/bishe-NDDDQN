...
from model import create_duel_q_network_bz,create_duel_q_network_dh, create_model
...
def get_fixed_samples(env, num_actions, num_samples):
    ...
    for _ in range(num_samples):
        old_state, old_action1, old_action2, action1_bz,action2_bz,action1_dh,action2_dh,reward_bz,reward_dh, new_state, is_terminal,is_arrived = env.GetState()
        action1 = random.randrange(num_actions)
        action2 = random.randrange(num_actions)
        env.TakeAction(action1, action2,action1_bz,action2_bz,action1_dh,action2_dh)
...
def main():
    ...
    online_model, online_params = create_model(args.window_size, args.input_shape, args.num_actions,
                    'online_model', create_duel_q_network_bz,create_duel_q_network_dh, trainable=True)
    target_model, target_params = create_model(args.window_size, args.input_shape, args.num_actions,
                    'target_model', create_duel_q_network_bz,create_duel_q_network_dh, trainable=False)
