...
class GazeboWorld():
    ...
    def GetRewardAndTerminate(self):
        ...
        reward_bz =2*v * v * np.cos(2 * v * theta)-0.1
        reward_dh=reward_distance+reward_angle
        if self.GetBump():
            reward_bz = -20.
            terminate = True
            reset = True
        ...
        return reward_bz,reward_dh, terminate, reset,arrived
    ...
    def GetReward_Angle(self,new_angle,old_angle):
        ...
        if abs(new_angle)<0.05:
            ...
            reward_angle=-0.2
        elif abs(new_angle)<0.7:
            reward_angle=-0.4
        else:
            reward_angle=-1.0
        return reward_angle
    def GetState(self):
        return np.copy(self.state['old_state']), self.state['action1'],   self.state['action2'],    \
                       self.state['action1_bz'], self.state['action2_bz'],self.state['action1_dh'], \
                       self.state['action2_dh'], self.state['reward_bz'] ,self.state['reward_dh'] , \
               np.copy(self.state['new_state']), self.state['is_terminal'],self.state['is_arrived']
    def TakeAction(self,action1,action2,action1_bz,action2_bz,action1_dh,action2_dh):
        robot_position=self.GetSelfState()
        ...
        reward_bz,reward_dh, is_terminal, reset ,is_arrived= self.GetRewardAndTerminate()
        ...
        self.state['action1_bz'] = action1_bz
        self.state['action2_bz'] = action2_bz
        self.state['action1_dh'] = action1_dh
        self.state['action2_dh'] = action2_dh
        self.state['reward_bz'] = reward_bz
        self.state['reward_dh'] = reward_dh
