...
#conv3:null,10,13,32
    ...
    conv1_b = tf.get_variable(shape=[16], name='conv1_b', trainable=trainable, initializer=tf.constant_initializer(
        0.05))
    conv1 = tf.nn.conv2d(input_frames1, conv1_W, strides=[1, 4, 4, 1],
    ...
    flatten_shape2 = input_frames2.get_shape()
    flat_output3_size = flatten_shape1[1] * flatten_shape1[2] * flatten_shape1[3]
    flat_output4_size = flatten_shape2[1] * flatten_shape2[2] * flatten_shape2[3]
    flat_output3 = tf.reshape(output3, [-1, flat_output3_size], name='flat_output3')
    flat_output4 = tf.reshape(input_frames2, [-1, flat_output4_size], name='flat_output4')
    fff=tf.concat([flat_output4,flat_output4,flat_output4,flat_output4,flat_output4,flat_output4],1)
    output_last=tf.concat([flat_output3,fff,fff,fff,fff,fff,fff,fff,fff,fff,fff],1)
    flatten_shape = output_last.get_shape()
    print(flat_output3.shape[1])
    print(flatten_shape[1])
    return flat_output3, flat_output3_size,flat_output4, [conv1_W, conv1_b, conv2_W, conv2_b, conv3_W, conv3_b]

# branching noisy dueling architecture
def create_duel_q_network(input_frames,num_actions,trainable):

    with tf.variable_scope('common'):
        flat_output, flat_output_size, flat_output4,parameter_list= create_conv_network(input_frames,trainable)
    outputA, parameter_list_outputA = noisy_dense(flat_output, name='fcA',
                                                 input_size=int(flat_output_size), output_size=512, trainable=trainable,
                                                 activation_fn=tf.nn.relu)
    outputA1=tf.concat([outputA,flat_output4],1)
    outputA2, parameter_list_outputA2 = noisy_dense(outputA, name='fcA2',
                                                    input_size=512, output_size=num_actions,
                                                    trainable=trainable)
    ouputA_2, parameter_list_outputA_2 = noisy_dense(flat_output, name='fcA_2',
                                                     input_size=int(flat_output_size), output_size=512, trainable=trainable,
                                                     activation_fn=tf.nn.relu)
    ouputA_2_1=tf.concat([ouputA_2,flat_output4],1)
    ...
    outputV, parameter_list_outputV = noisy_dense(flat_output, name='fcV',
                                                  input_size=int(flat_output_size), output_size=512, trainable=trainable,
                                                  activation_fn=tf.nn.relu)
    outputV1=tf.concat([outputV,flat_output4],1)
    ...
    q_network = tf.nn.relu(outputV2 + outputA2 - tf.reduce_mean(outputA2, 1, keep_dims=True), name='q_network')
    q_network_2 = tf.nn.relu(outputV2 + outputA2_2 - tf.reduce_mean(outputA2_2, 1, keep_dims=True), name='q_network_2')
