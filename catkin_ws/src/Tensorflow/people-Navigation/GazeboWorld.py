...
class GazeboWorld():
    ...
    def GetPointObservation(self):
        ...
        angle=abs(self.get_relative_angle_to_goal(radian_or_degree='radian')/np.pi)
        point=np.zeros(shape=(1,self.input_shape[1]))
        point[0][self.input_shape[1]-4]=distance
        point[0][self.input_shape[1]-3]=angle
        point[0][self.input_shape[1]-2]=robot_position[0]-self.goal_position.point.x
        point[0][self.input_shape[1]-1]=robot_position[1]-self.goal_position.point.y
        return point
    ...
    def SetRobotPose(self):
        quaternion = tf.transformations.quaternion_from_euler(0., 0., np.random.uniform(-np.pi*0.5, -np.pi*0.3))
    ...
    def GetReward_Distance(self,new_distance,old_distance):
        ...
        if new_distance<old_distance:
            reward_distance=0.1*(12-new_distance)
